...
def objective(trial):
    ...
    M = trial.suggest_categorical("M", [500, 768, 1024])
    L = trial.suggest_categorical("L", [128, 256])
    model_choice = trial.suggest_categorical("model_choice", ["attention", "gated_attention"])
    epochs = trial.suggest_int("epochs", 5, 6)
    ...
    wandb.init(project="MIL_MultiTask_newcontrast_6cancers", config=hyperparameters, reinit=True)
    global args
