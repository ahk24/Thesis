        ...
        for batch in train_loader:
            ...
            with torch.amp.autocast(device_type='cuda'):  # Mixed precision forward pass
                ...
                if len(bag_reps) > 1:
                    ...
                    contrast_loss = supervised_contrastive_loss(bag_reps_tensor, aux_labels_tensor)
                else:
...
def objective(trial):
    ...
    aux_weight = trial.suggest_float("aux_weight", 0.1, 1.0)
    ...
    wandb.init(project="MIL_MultiTask_Prism", config=hyperparameters, reinit=True)
    global args
