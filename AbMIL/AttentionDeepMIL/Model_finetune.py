...
class Attention(nn.Module):
    def __init__(self, input_dim, M=500, L=128, attention_branches=1):
        ...
        # Updated classifier with two additional linear layers.
        self.classifier = nn.Sequential(
            nn.Linear(self.M * self.ATTENTION_BRANCHES, self.M),  # first linear layer
            nn.ReLU(),
            nn.Linear(self.M, self.M // 2),                        # second linear layer
            nn.ReLU(),
            nn.Linear(self.M // 2, 1),                             # third linear layer
