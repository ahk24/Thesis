...
def main():
    ...
    concatenated_data_dir = "/data/temporary/amirhosein/model_script_concatenated"
...
def sample_config():
    ...
    config = {}
    config["lr"] = 10 ** np.random.uniform(-5, -3)  # between 1e-5 and 1e-3
    config["weight_decay"] = 10 ** np.random.uniform(-6, -3)  # between 1e-6 and 1e-3
...
def run_cv_experiment(config, max_epochs):
    """
    Run the 3-fold CV experiment with the given hyperparameter configuration
    using a limited number of epochs (budget). Returns the average best validation loss.
    """
...
def final_retrain(config):
    """
    Retrain the model using the best hyperparameters on the full 3-fold CV setup.
    Saves the best model (per fold) to disk and logs final performance.
    """
...
def hyperband_tuning():
    """
    Implements Hyperband to tune hyperparameters.
    After tuning, the best configuration is used to final-retrain the model.
    """
    R = 10  # maximum epochs
    eta = 3
    ...
    # Final retraining using the best configuration
